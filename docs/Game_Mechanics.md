...
## Game Mechanics (from C++ server source)
- Map: 500×500 (smallMap), nests at corners, ~18,990 food tiles (never respawn)
- Config: timeScale=4, startingFood=60, hungerRate=1, foodYield=1, foodTheftYield=3, all other mods=1
- Food drain: `foodCount -= delta × hungerRate` each frame (delta = realtime_elapsed × timeScale). At timeScale=4, hungerRate=1: ~4 food/real-sec
...
## `agc.delta` Property
- `agc.delta` returns the game-time elapsed since last frame as a `float` (seconds × timeScale)
- Computed as `(now - lastFrame).count() * timeScale` BEFORE frame callbacks run
- Use for time-based triggers instead of frame counting (frame rate varies with load)
- Example: accumulate `agc.delta` to track elapsed game-time for all-in timing

## Callback Reference
Callback order per frame: `antNew → antDeath → gameFrame → antHit → antHurt → antFull → antGrab → antDeliver → antWait`

- `gameStart`: called once at start. 5 DEFAULT ants already exist
- `gameFrame`: called every frame. Main logic loop
- `antNew`: called when new ant created. Called BEFORE `gameFrame` in the same frame
- `antDeath`: called when ant dies. Ant is frozen snapshot; do NOT send commands. Called BEFORE `gameFrame`
- `antHit`: called when a friendly ant's attack lands on an enemy (AINTERACT succeeds)
- `antHurt`: called when ant takes damage
- `antFull`: called when ant reaches max carry capacity
...
- Food theft at enemy nests: Python-side `take`/`goTake` requires ant to not be full (`capacity <= foodCarry`). Server-side yields `min(foodTheftYield, capacity - foodCarry)`. Ant must walk to nest tile and use `take`/`goTake`
